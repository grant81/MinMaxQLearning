...
class MiniMax_Q_Agent:
    ...
    def update_PI(self, state):
        ...
        for i in range(1,len(res.x)):
            if res.x[i]<0:
                res.x[i] = 0
        if res.success:
            self.PI[state] = res.x[1:]/res.x[1:].sum()
    ...
    def save_agent(self,path = MODEL_PATH):
        ...
        PI.to_csv(path + 'PI_'+str(self.id)+'.txt', sep=' ', header=False, index=False)
    ...
    def load_agent(self,path = MODEL_PATH):
        PI = pd.read_csv(path + 'PI_'+str(self.id)+'.txt', sep=' ', header=None)
